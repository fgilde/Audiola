...
public sealed partial class TimelineViewModel : ObservableObject
{
    ...
    public IAdvancedSeparationService AdvSep => _advSep ??= AppServices.Get<IAdvancedSeparationService>();
    private ExportService? _export;
    private ExportService Export => _export ??= AppServices.Get<ExportService>();
    private SongMetadata? _songMeta;
    private SongMetadata SongMeta => _songMeta ??= AppServices.Get<SongMetadata>();
    ...
    /// <summary>Playhead-Position in Pixeln (die Views setzen daraus Margin bzw. Canvas.Left).</summary>
    ...
    {
        var picked = await _files.OpenFilesAsync("Audiodatei(en) als Spur hinzufügen", true,
    ...
    {
        if (track is null) return;
        _dialogs.ShowTrackMastering(track);
    }
    ...
    private void OpenSingAlong()
    {
        ...
        _dialogs.OpenSingAlong();   // eigenes Fenster, nicht-modal – kann neben dem Studio liegen
    }
    ...
    private void UpdatePlayhead()
        => PlayheadX = _engine.Position.TotalSeconds * PixelsPerSecond;
