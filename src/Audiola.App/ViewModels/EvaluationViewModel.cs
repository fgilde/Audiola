...
public sealed partial class EvaluationViewModel : ObservableObject
{
    ...
    public EvaluationViewModel(IEvaluationService eval, IFileDialogs files)
    {
        _eval = eval;
        _files = files;
    }
    ...
    {
        var path = await _files.OpenFileAsync("Detektor-Ergebnisse (CSV) laden",
            new FileFilter("CSV-Datei", "csv", "tsv", "txt"), FileFilter.Any);
        if (path is null) return;

        try
        {
            _rows = await _eval.ParseAsync(path);
