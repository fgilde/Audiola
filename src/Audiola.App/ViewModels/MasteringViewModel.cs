...
public sealed partial class MasteringViewModel : ObservableObject
{
    ...
    // Als Punkte-String, weil WPF (PointCollection) und Avalonia (Points) beide dessen
    // TypeConverter für Polyline.Points verstehen — ein Typ für beide Hosts.
    [ObservableProperty] private string _eqCurve = "";
    ...
    public MasteringViewModel(SessionState session, IMasteringService mastering, INotifier snackbar,
        TimelineViewModel timeline, StemMixerEngine engine, LiveMasterProcessor liveMaster, TransportViewModel transport,
        ISettingsService settings, IFileDialogs files)
    {
        Session = session;
    ...
    private void UpdateEqCurve()
    {
        ...
        for (var i = 0; i <= n; i++)
        {
            ...
            if (i > 0) pts.Append(' ');
            pts.Append((t * w).ToString("0.##", CultureInfo.InvariantCulture)).Append(',')
               .Append(y.ToString("0.##", CultureInfo.InvariantCulture));
        }
        EqCurve = pts.ToString();
    }
    ...
    {
        var picked = await _files.OpenFilesAsync("Dateien für Bulk-Mastering wählen", true,
        ...
        try
        {
            ...
            {
                var input = files[i];
                BulkStatus = $"Mastere {i + 1}/{files.Count}: {Path.GetFileName(input)} …";
                try
                {
            ...
            if (fail == 0) _snackbar.Success("Bulk-Mastering fertig", BulkStatus, 5);
            else _snackbar.Warning("Bulk-Mastering fertig", BulkStatus, 5);
        }
        finally
        {
            IsBulkRunning = false;
    ...
    private async Task ExportAsync()
    {
        ...
        var src = _sourcePath;
        var settings = BuildSettings();
        var meta = AppServices.Get<SongMetadata>().ToMetadata();
        var name = string.IsNullOrWhiteSpace(meta.Title) ? "studio-master" : meta.Title!;

        await AppServices.Get<Audiola.Services.ExportService>().ExportAsync(
            name,
            async () =>
            {
