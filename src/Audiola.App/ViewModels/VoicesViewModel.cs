...
public sealed partial class VoicesViewModel : ObservableObject
{
    ...
    public VoicesViewModel(ILocalVoiceService local, IVoiceProfileStore store, ISettingsService settings,
        IAudioRecorder recorder, INotifier snackbar, IFileDialogs files)
    {
        _local = local;
        _store = store;
        _settings = settings;
    ...
    {
        var path = await _files.OpenFileAsync("Stimm-Sample wählen", FileFilter.Audio, FileFilter.Any);
        if (path is not null) SamplePath = path;
    }
