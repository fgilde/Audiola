...
public sealed partial class SingAlongViewModel : ObservableObject, IDisposable
{
    ...
    public SingAlongViewModel(TimelineViewModel timeline, ISettingsService settings,
        SongMetadata songMeta, INotifier snackbar, ILocalVoiceService voice, IVoiceProfileStore voiceStore)
    {
        _timeline = timeline;
        _settings = settings;
        _songMeta = songMeta;
    ...
    private void OnPlaybackEnded()
    {
        ...
        DispatcherHelper.PostToUi(() => { IsPlaying = false; IsRecording = false; });
    }
    ...
    private void OnPitchDetected(object? sender, PitchSample s)
    {
        ...
        if (!DispatcherHelper.IsOnUiThread) { DispatcherHelper.PostToUi(() => OnPitchDetected(sender, s)); return; }
